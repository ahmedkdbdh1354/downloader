...
export default function RecentList({ items, loading }) {
  return <section className="mx-auto mt-20 max-w-5xl">
    ...
    <div className="overflow-hidden rounded-2xl border border-white/[.08] bg-white/[.025]">
      {loading ? <div className="px-5 py-10 text-center text-sm text-slate-500">جارٍ تحميل السجل…</div> : items.length ? items.map((item, index) => <div key={item.id} className={`flex items-center gap-3 px-4 py-3.5 sm:px-5 ${index ? 'border-t border-white/[.06]' : ''}`}>
